...
3. **로컬 DB 기반 완벽한 통계 및 성적표**
   ...
   - 🎯 **정밀 당첨 확인 및 알리미**: 동행복권의 당첨 번호(보너스 포함)를 스크래핑하여, 내가 산 로또 번호와 정확히 **파란색/자주색 브라켓(`[02]`) 하이라이팅**으로 교집합 매칭을 보여주며 1등~5등을 채점합니다.
   - 🛡️ **차단(WAF) 회피 자동화**: 주말 접속자 폭주로 인한 대기열 페이지를 만나도 에러를 뿜지 않고 젠틀하게 스킵, 다음 자동화 주기를 기다리도록 예외가 완벽히 처리되어 있습니다.
   - 📊 **평생 누적 성적표**: 지금까지 이 프로그램을 통해 사용한 모든 지출액, 누적 당첨금, 랭크별 당첨 횟수 및 **종합 순수익(Net Profit)** 을 산출합니다.
...
### 📊 당첨 확인 및 장부(통계) 관리
```bash
# 최신 당첨 결과 갱신 (보통 백그라운드 Crontab에 걸어둠)
# 미확인된 '추첨 전' 티켓들의 실제 당첨 여부를 조회하고 DB에 업데이트합니다.
python main.py update

# (설렘 가득!) 내가 아직 확인 안 한 새로운 티켓의 당첨 결과를 봅니다.
# 동행복권 앱과 동일하게 [당첨 번호]는 파란색, [보너스]는 자주색으로 터미널 UI에 예쁘게 색칠해줍니다!
python main.py check-pending
> [출력 예시]
> [13] [15] [16] [33] [40] [42]  =>  3등 (1,500,000원)
